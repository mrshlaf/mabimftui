  ...
  return (
    <>
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Rujukan di sekitar Depok dan Jakarta Timur.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          <button
            ...
            onClick={() => scrollBy(-1)}
            disabled={index === 0}
            className="grid h-9 w-9 place-items-center rounded-full bg-secondary text-foreground ring-1 ring-border transition-[transform,color,background-color] active:scale-95 hover:bg-card hover:text-accent disabled:pointer-events-none disabled:opacity-40"
          >
      ...
      <div
        ...
        className="mt-4 flex scroll-smooth snap-x snap-mandatory gap-3 overflow-x-auto pb-2"
      >
        {rumahSakitTerdekat.map((rs) => (
          <article
            key={rs.nama}
            className="w-[15rem] shrink-0 snap-start rounded-2xl bg-secondary/60 p-4"
          >
            <div className="flex items-center gap-2.5">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-card text-accent shadow-card">
                <Hospital className="h-4 w-4" />
              </span>
              <p className="text-sm font-semibold text-foreground">{rs.nama}</p>
            </div>
            ...
            <a
              href={`tel:${rs.telp}`}
              className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-card px-3 py-1 text-xs font-semibold text-accent ring-1 ring-accent/30 transition-colors hover:ring-accent/60"
            >
